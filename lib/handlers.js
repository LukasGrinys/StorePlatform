...
userAuth = (cookie, callback) => {
  ...
  if (token) {
    ...
    callback(false, "No token was sent");
  }
  ...
  if (data.method === "get") {
    ...
    userAuth(str, (isAuth, msg) => {
      if (isAuth) {
        ...
        helpers.getTemplate('login', null, function(err,str) {
          if (!err && str) {
            callback(200, str, 'html');
          } else {
            callback(500, undefined, 'html');
...
handlers.index = (data,callback) => { getPublicPage('index', data, callback); };
handlers.checkOrder = (data,callback) => { getPublicPage('checkOrder', data, callback); }
handlers.dashboardPage = (data,callback) => { getAdminPage('dashboard', data, callback)};
          ...
          if (!err && info) {
            ...
            if (allProducts.length === files.length) {
              ...
              for (let i = 0; i < orderInfo.length; i++) {
                ...
                let priceNum = Number((item.quantity * item.price * 100));
                totalPrice += priceNum;
              ...
              stripe.charges.create({
                ...
                _data.create('orders', orderId, obj, function(err) {
                  if (!err) {
                    ...
                    callback(500, { message : err, success: false});
                  }
                })
              }).catch( (err) => {
...
handlers._products.post = (data, callback) => {
    ...
    userAuth(cookie, (isAuth, msg) => {
      if (isAuth) {
        ...
        let imageSrc = typeof(body.imageURL) === "string" && body.imageURL.trim().length > 0 ? body.imageURL.trim() : 'Items/default.jpg';
          ...
          fs.readdir('./.data/products', (err, files) => {
            ...
            for (let i = 0; i < len; i++) {
              ...
              if (numArr.length === len) {
                ...
                let lastChangesBy = parsedAuth.username;
                const dataObj = {
                  id : newId,
                  name : title,
                  category : category,
...
handlers._products.put = (data, callback) => {
  ...
  userAuth(cookie, (isAuth, msg) => {
      if (isAuth) {
        ...
        let title = body.title.trim().length > 0 && typeof(body.title) === "string" ? body.title.trim() : false;
        let category = body.category.trim().length > 0 && typeof(body.category) === "string" ? body.category.trim() : false;
        let imageSrc = body.imageURL.trim().length > 0 && typeof(body.imageURL) === "string" ? body.imageURL.trim() : 'Items/default.jpg';
        let description = body.description.trim().length > 0 && typeof(body.description) === "string" ? body.description.trim() : false;
        ...
        if (title && category && description && price && itemId) {
            let timeOfChanges = Date.now();
            let lastChangesBy = helpers.parseJsonToObject(cookie).auth.username;
            const dataObj = {
                  id : itemId,
                  name : title,
                  category : category,
    ...
    if (isAuth) {
      ...
      if (userId) {
        ...
        _data.read('users', userId, (err, userData) => {
          if (!err && userData) {
            if (userData.username === username || username === "admin" || username === "devadmin") {
